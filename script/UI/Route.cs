...
public partial class Route : Control
{
	...
	{
		const float sizex = 250 / 2, spacex = 100;
	...
	/// </summary>
	//[Export] 
	public float Cardy(int rowCount, int row)
	{
	...
	public override void _Ready()
	{
		...
		for (int i = 0; i < cardLv.Length; i++)
		{
			...
			for (int j = 0; j < cardLv[i].Length; j++)
			{
				...
				// 3.2 设置关卡位置
				cardLv[i][j].Position = new Vector2(Cardx(i) + 100, Cardy(rowCount, j));
				// 3.3 设置关卡的父容器
		...
		Control b = new()
		{
			CustomMinimumSize = new Vector2(cardLv.Length * 225 + 400, 0) // 宽度计算，留有余量
		};
	...
	private void Link()
	{
		...
		for (int i = 1; i < cardLv.Length; i++)
		{
			...
			{
				cardLv[i - 1][0].Link(cardLv[i][0], 0);
				cardLv[i][0].PreCount++;
				continue;
			}
			...
			// 初始化连接列表
			for (int j = 0; j < lines.Length; j++) lines[j] = [];
			for (int j = 0; j < revlines.Length; j++) revlines[j] = [];
			...
			while (s.Count > 0)
			{
				...
				int RandomT(int tf, int tl)
				{
					int x = WeightedRandom(from _ in Enumerable.Range(tf, tl - tf) select (_, MathF.Pow(1.1f, -MathF.Abs(cardLv[i - 1][f].Position.Y - cardLv[i][_].Position.Y))));
					/*
					foreach (float i in from _ in Enumerable.Range(tf, tl - tf) select MathF.Pow(1.1f, -MathF.Abs(cardLv[i - 1][f].Position.Y - cardLv[i][_].Position.Y)))
					{
						Debug.WriteLine(i);
					}
					Debug.WriteLine($"{MathF.Pow(1.1f, -MathF.Abs(cardLv[i - 1][f].Position.Y - cardLv[i][x].Position.Y))};");
					*/
					return x;
				}
				if (ff == f) // 分割点在分区最左
			...
			for (int j = 0; j < lines.Length; j++)
			{
				...
				{
					// 添加到前一关卡的Nexts列表
					cardLv[i - 1][j].Link(cardLv[i][k], revlines[k].IndexOf(j));
